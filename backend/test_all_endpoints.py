...
def run_tests():
    ...
    # 2. Dashboard Metrics
    dash_res = httpx.get(f"{BASE_URL}/dashboard/metrics", headers=headers)
    ...
    # 3. Menu Items
    menu_res = httpx.get(f"{BASE_URL}/menu", headers=headers)
    assert menu_res.status_code == 200, f"Menu failed: {menu_res.text}"
    menu_items = menu_res.json()["data"]["items"]
    ...
    order_res = httpx.post(f"{BASE_URL}/orders", json=order_payload, headers=headers)
    assert order_res.status_code in [200, 201], f"Order creation failed: {order_res.text}"
    new_order = order_res.json()["data"]
    print(f"[PASS] 4. Order Creation (POS): HTTP {order_res.status_code} - Created Order #{new_order['id']} Total: Rs.{new_order['total_amount']}")

    # 5. Get Orders List
    orders_res = httpx.get(f"{BASE_URL}/orders", headers=headers)
    assert orders_res.status_code == 200, f"Orders list failed: {orders_res.text}"
    orders_list = orders_res.json()["data"]["orders"]
    print(f"[PASS] 5. Orders History: HTTP 200 OK - {len(orders_list)} orders in database")

    # 6. Customers List
    cust_res = httpx.get(f"{BASE_URL}/customers", headers=headers)
    assert cust_res.status_code == 200, f"Customers failed: {cust_res.text}"
    customers = cust_res.json()["data"]["customers"]
    print(f"[PASS] 6. Customers: HTTP 200 OK - {len(customers)} customers retrieved")

    # 7. Reviews & Sentiment
    rev_res = httpx.get(f"{BASE_URL}/reviews", headers=headers)
    assert rev_res.status_code == 200, f"Reviews failed: {rev_res.text}"
    reviews = rev_res.json()["data"]["reviews"]
    print(f"[PASS] 7. Reviews: HTTP 200 OK - {len(reviews)} reviews retrieved")

    # 8. Food Waste
    waste_res = httpx.get(f"{BASE_URL}/waste", headers=headers)
    assert waste_res.status_code == 200, f"Waste failed: {waste_res.text}"
    waste_items = waste_res.json()["data"]["items"]
    ...
    df_res = httpx.get(f"{BASE_URL}/analytics/demand-forecast", headers=headers)
    assert df_res.status_code == 200, f"Demand forecast failed: {df_res.text}"
    print(f"[PASS] 11. ML Demand Forecast: HTTP 200 OK - {len(df_res.json().get('items', []))} items forecasted")

    # 12. ML Customer Segments
    cs_res = httpx.get(f"{BASE_URL}/analytics/customer-segments", headers=headers)
    assert cs_res.status_code == 200, f"Customer segments failed: {cs_res.text}"
    print(f"[PASS] 12. ML Customer Segments: HTTP 200 OK - {len(cs_res.json().get('segments', []))} segments")

    # 13. ML Anomalies
    anom_res = httpx.get(f"{BASE_URL}/analytics/anomalies", headers=headers)
    assert anom_res.status_code == 200, f"Anomalies failed: {anom_res.text}"
